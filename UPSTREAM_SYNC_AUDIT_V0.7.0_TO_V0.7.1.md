...
| 43 | [`80d3d2d`](https://github.com/CookSleep/gpt_image_playground/commit/80d3d2d) | 2026-07-15 | 部分可审核 | 提取任务完成、失败、中断、实际参数与提示词映射等状态转换。适合本地重构，但需保留 OpenAI 兼容及自定义服务商逻辑并去除 fal/Agent 分支。 |  |
...
| 51 | [`971e444`](https://github.com/CookSleep/gpt_image_playground/commit/971e444) | 2026-07-21 | 可审核 | 支持一次选择多份普通 ZIP 备份并合并导入，同时保持分片备份完整性校验。当前项目已有 v0.7.0 分片基础，可按本地备份格式移植并补测试。 |  |
...
1. 建立独立升级分支，并在不处理现有未提交文件的前提下固定当前 `96` 项测试与生产构建基线。
2. 第一阶段只做行为不变的模块提取：图片缓存、任务状态、设置页子组件；每个模块单独提交并运行全量测试、构建。
3. 第二阶段移植用户可见功能：多普通备份合并导入、参考图预览内替换/遮罩编辑、遮罩编辑器交互优化。
4. 第三阶段处理高风险内部逻辑：本地输入草稿、精简持久化模块、任务删除事务；每项独立提交，不与普通代码搬迁混合。
5. `contentEditable` 与 SSE 解析器按现有测试缺口决定是否在本轮完成；若实施，先补行为测试，再移动代码。
6. 完成选定条目后，再更新 `package.json`、`package-lock.json` 到 `0.7.1`，更新本审核文档的“人工审核”列，并进行完整回归。
...
4. 保留 OpenAI 兼容 Images API、自定义服务商、`stream: true`、`partial_images`、本地代理和现有部署策略。
...
1. `v0.7.1` 的核心重构不能整版合并，但图片缓存、任务状态、设置页子组件等模块边界适合在当前项目中等价复刻。
2. 上游 `store.ts` 的大幅缩减包含大量当前项目早已删除的 Agent、Responses、收藏夹和输入草稿代码，本地不能预期获得相同的净减行数。
3. 本轮优先目标应是降低后续上游同步冲突并提高可测试性，而不是追求目录或文件数量与上游完全一致。
4. 在人工审核完成前，不创建实施提交、不修改源码、不更新版本号。
